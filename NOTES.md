...
| Cold-start resume | First cut: timestamp-compare local vs the service's `not_started` default | **Rejected (QA-caught)** | The in-memory service returns a fresh-stamped `not_started` that beat the persisted draft → resume lost everything. Fixed: `not_started` server ⇒ resume the local draft. Regression test added. |
| In-app poll error handling | First cut: `POLL_ERROR` set `machineStatus:'error'`; `active` gated on it | **Rejected (QA-caught)** | One transient poll error tore down the loop, so the hook's error-bound was dead in the app though E8 passed in isolation. Fixed: `POLL_ERROR` stays `polling`; added an App integration test (resume → poll error → recover to approved). |
